...
//const char* cStr2 = testString.c_str();
...
uint8_t function_behavior_count = 0; //cremaker Pro 2.1 added
...
uint16_t max_display_update_time = 0;
  ...
  void lcd_control_motion_menu();
    ...
    //void lcd_control_temperature_preheat_material2_settings_menu(); //cremaker 2.1 menu delete
  #endif
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_sdcard_pause() {
      ...
      print_job_timer.pause();
		...
		enqueue_and_echo_commands_P(PSTR("M17")); //cremaker 2.1 modified
      #endif
      lcd_reset_status();
    }
    ...
    void lcd_sdcard_resume() {
		...
		enqueue_and_echo_commands_P(PSTR("M84"));//cremaker 2.1 modified
      #else
        card.startFileprint();
  ...
  void layer_height_setting() {
	enqueue_and_echo_commands_P(PSTR("G28 X Z\n"
		"G0 F3000 Z0.3\n"
		"M300 S440 P200\n"
		"M84\n"
	    "M117 Check Leveling..."));
  ...
  void z_horizontal_cali() {

	  defer_return_to_status = true;

	  START_SCREEN();
	  STATIC_ITEM(MSG_Z_HORIZONTAL_CALI_PUSH_1);
	  STATIC_ITEM(MSG_Z_HORIZONTAL_CALI_PUSH_2);
	  STATIC_ITEM(MSG_Z_HORIZONTAL_CALI_PUSH_3);
	  END_SCREEN();

	  if (function_behavior_count == 0) {
		  enqueue_and_echo_commands_P(PSTR("G28 X Z\n"));
		  function_behavior_count++;
	  }

	  if (function_behavior_count == 1) {
		  //Z_height_top_over_move_to();

		  //do_blocking_move_to_z(Z_MAX_POS_2);
		  //enqueue_and_echo_commands_P(PSTR("G28 X Z\n" "G0 F6000 Z320"));

		  //lcd_return_to_status();//Test
	  }
	  /*
	  if ((READ(Z_MIN_PIN) == true) && current_position[Z_AXIS])
	  {
		  Z_height_top_over_move_to();
	  }
	  */
	  ...
	  if (use_click()) {
		  Z_height_top_over_move_to();
		  enqueue_and_echo_commands_P(PSTR("G28 Z\n" "M117 Finished Z-Cali"));
		  function_behavior_count = 2;
	  }
	  
	  if (function_behavior_count == 2) {
		  function_behavior_count = 0;
		  return lcd_return_to_status();
	  }
  }
  //cremaker Pro 2.1 added

  void first_layer_test() {
	enqueue_and_echo_commands_P(PSTR("M300 S440 P200\n"
		"M104 S200\n"
		"M140 S60\n"
		"M190 S60.000000\n"
		"M109 S200.000000\n"
		...
		"M107\n"
		"G1 F4200 E-4.000\n"
		"G1 F3000 X21.0 Y20.2 Z0.3\n"
		"G1 F4200 E0.000\n"
		"G1 F1080\n"
		"G1 X181.33 Y20.2 E7.9624\n"
		"G1 X181.33 Y69.8 E10.4369\n"
		"G1 X21.33 Y69.8 E18.4193\n"
		"G1 X21.33 Y130.2 E21.4327\n"
		"G1 X181.33 Y130.2 E29.4151\n"
		"G1 X181.33 Y179.8 E31.8896\n"
		"G1 X18.67 Y179.8 E40.0048\n"
		"G0 X0.0 Y190 Z10\n"
		"G91\n"
		"G1 E-1 F300\n"
		"M84\n"
  ...
  //cremaekr 2.1 added
    ...
    void lcd_preheat_m1_menu() {
      ...
      #if HOTENDS == 1
        #if HAS_HEATED_BED
          MENU_ITEM(function, MSG_PREHEAT_1_ALL, lcd_preheat_m1_e0); //cremaker 2.1 modified
    ...
    void lcd_preheat_m2_menu() {
      ...
      MENU_BACK(MSG_PREPARE);
          ...
          MENU_ITEM(function, MSG_PREHEAT_2_END, lcd_preheat_m2_e0_only); //cremaker 2.1 modified
        #else
  ...
  void lcd_prepare_menu() {
    ...
    //
      ...
      // Set Home Offsets
      //
      //MENU_ITEM(function, MSG_SET_HOME_OFFSETS, lcd_set_home_offsets); //cremaker 2.1 menu delete
    #endif

    //
    // Disable Steppers
    //
    //MENU_ITEM(gcode, MSG_DISABLE_STEPPERS, PSTR("M84")); //cremaker 2.1 menu delete

	//
	// Leveling Z 0.3 Hight
	// 
	MENU_ITEM(function, MSG_LEVELING_HIGHT, layer_height_setting); //cremaker 2.1 menu
	
	//
	// Z Horizontal Calibration
	//
	MENU_ITEM(submenu, MSG_Z_HORIZONTAL_CALIBRATION, z_horizontal_cali); //cremaker Pro 2.1 menu

	//
	// Test _Leveling
	//
	MENU_ITEM(function, MSG_FIRST_LAYER, first_layer_test);	//cremaker 2.1 menu
   ...
   */
    ...
    #if ENABLED(PIDTEMP)
      int16_t autotune_temp[HOTENDS] = ARRAY_BY_HOTENDS1(200); //cremaker 2.1 modified
    #endif
  ...
  #endif // PID_AUTOTUNE_MENU///cremaker 2.1 menu
    ...
    //
      ...
      #define _PID_BASE_MENU_ITEMS(ELABEL, eindex) \
        ...
        #define PID_MENU_ITEMS(ELABEL, eindex) \
          ...
          MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int3, MSG_PID_AUTOTUNE ELABEL, &autotune_temp[eindex], 150, heater_maxtemp[eindex] - 15, lcd_autotune_callback_E ## eindex) //cremaker 2.1 pid autotune
      #else
